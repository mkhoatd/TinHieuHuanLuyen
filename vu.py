...
def plot_voiced_unvoiced(audio_name, signal, t, timestamp_label, STE, ZCR, VU, renderer='notebook'):
    ...
    names = {'Plot 1': 'Đồ thị STE và ZCR', 'Plot 2': 'Đồ thị Voice-Unvoice chuẩn',
             'Plot 3': 'Đồ thị Voice-Unvoice do thuật toán', 'Plot 4': 'Đồ thị so sánh kết quả đạt được'}
    # fig.for_each_annotation(lambda a: a.update(
    #     text=a.text + ': ' + names[a.text]))
    fig.add_annotation(xref='x domain', yref='y domain', x=0.5, y=1.25, showarrow=False, text='<b>Đồ thị STE và ZCR</b>', row=1, col=1)
...
def voiced_unvoiced(audio_name_list, plot_name_list):
    ...
    for i in range(len(audio_name_list)):
        signal_list[i], sr_list[i], t_list[i], timestamp_label_list[i] = load_data(
            audio_name_list[i])
        signal_frames_list[i], time_frames_list[i], frame_size_list[i], frames_count_list[i] = separate_frames(
    ...
    for i in range(len(audio_name_list)):
        ...
        if audio_name_list[i] in plot_name_list:
            plot_voiced_unvoiced(audio_name_list[i], signal_list[i], t_list[i],
                                 timestamp_label_list[i], STE_norm_T_list[i], ZCR_norm_T_list[i], VU_list[i], renderer='svg')
